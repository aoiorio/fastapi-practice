...
async def read_all(user: user_dependency, db: db_dependency):
    ...
    """
    I think user is a dictionary, because I defined user as a dictionary.
...
async def create_todo(
    ...
    # check user is now authorized, if not yet, raise an error
    if user is None:
        raise HTTPException(status_code=401, detail="😩 Authentication Failed")
...
async def update_todo(
    ...
    # Find the value that matches todo_id and the id of current user.
    todo_model = (
        db.query(Todos)
        .filter(Todos.id == todo_id)
        .filter(Todos.owner_id == user.get("id"))
